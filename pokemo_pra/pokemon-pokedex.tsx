    ...
    <div className="min-h-screen bg-gradient-to-b from-red-600 via-red-500 to-red-700 relative">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -right-20 top-10 opacity-10 rotate-12">
          <img src="/placeholder.svg?height=400&width=400" alt="" className="w-96 h-96" />
        </div>
        <div className="absolute -left-20 top-1/3 opacity-10 -rotate-12">
          <img src="/placeholder.svg?height=300&width=300" alt="" className="w-72 h-72" />
        </div>
        <div className="absolute right-1/4 bottom-1/4 opacity-10 rotate-45">
          <img src="/placeholder.svg?height=200&width=200" alt="" className="w-48 h-48" />
        </div>
      </div>
      ...
      <div className="container mx-auto py-12 px-4 relative z-10">
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-8 shadow-xl">
          <h1 className="text-4xl font-bold text-center mb-2 text-red-600">ポケモン図鑑</h1>
          <p className="text-center text-muted-foreground mb-8">全国のポケモンを探そう！</p>
          ...
          <div className="max-w-4xl mx-auto mb-8">
            <div className="flex flex-wrap gap-2 justify-center">
              {pokemonTypes.map((type) => (
                <Badge
                  key={type.name}
                  className={`${getTypeColor(type.nameJa)} cursor-pointer hover:opacity-80 transition-opacity border-0 text-white`}
                >
                  {type.nameJa}
                </Badge>
              ))}
              ...
              >
                ...
                <CardContent className="p-3">
                  <div className="space-y-1.5">
                    <h2 className="font-bold text-sm line-clamp-1">{pokemon.name}</h2>
                    <div className="flex flex-wrap gap-1">
                      {pokemon.type.split("・").map((type, index) => (
                        <Badge key={index} className={`${getTypeColor(type)} border-0 text-white text-xs shadow-sm`}>
                          {type}
                        </Badge>
                      ))}
                    </div>
